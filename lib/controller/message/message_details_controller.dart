...
class MessageDetailsController extends GetxController {
  ...
  Future<void> sendMessageInGroup({required String docId, required dynamic messageText, required MessageType type}) async {
    if (messageText.isNotEmpty) {
      groupChatBotController.value.text = '';
      print('DOC ID 🎗 $docId');
      await MessageService().sendMessageInGroup(messageText: messageText, type: type, docId: docId);
    } else {
      print('chat boot is empty');
    }
  }
